...
@Component({
  selector: 'app-retreat-cart',
  templateUrl: './retreat-cart.component.html',
  styleUrls: ['./retreat-cart.component.scss']
})
export class RetreatCartComponent implements OnInit {
  ...
  personnalInformationMessageSuccess = [_('retreat-cart.infos.success')];
  ...
  universityFields = [
    ...
    },
    {
      name: 'text-warning',
      type: 'alert',
      label: _('retreat-cart.labels.personnal_infos')
    },
    {
      name: 'student_number',
      type: 'text',
  ...
  submitPersonalInformation() {
    ...
    this.userService.update(profile.url, value).subscribe(
      user => {
        ...
        this.personnalInformationMessageSuccess = [_('retreat-cart.infos.success_update')];
        this.defineCurrentStep();
      },
      err => {
        if (err.error.non_field_errors) {
