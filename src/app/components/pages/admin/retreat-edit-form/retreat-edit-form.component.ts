...
export class RetreatEditFormComponent implements OnInit {
      ...
      (params: Params) => {
        this.retreatService.get(params['id']).subscribe(
          data => {
            ...
            this.typeUrl = this.selectedRetreat.type.url;
  ...
  changeType(event) {
    for (const type of this.listRetreatType) {
      if (type.url === event.target.value) {
        this.type = type;
        this.typeUrl = event.target.value;
        this.initRetreatForm();
      }
    }
  }
    ...
    if (this.selectedRetreat) {
      ...
      this.retreatService.create(value).subscribe(
        () => {
          ...
          this.router.navigate(['/admin/retreats/type/' + this.type.id]);
        },
        err => {
          if (err.error.non_field_errors) {
            this.retreatErrors = err.error.non_field_errors;
  ...
  private refreshRetreatTypes() {
    this.retreatTypeService.list().subscribe(
      (retreatTypes) => {
        this.listRetreatType = retreatTypes.results.map(
